...
Voraussetzung sind die `plt` Binaries und Shell-Skripte und dass sie im `$PATH` liegen, also ein `export PATH=<repo path>/bin:$PATH` oder `export PATH=<repo path>/build/Debug/bin:$PATH`.

> ⚠️ In der `zsh` nach `<repo path>` wechseln.

> ⚠️ Den Pfad für die Executables setzen:

(Build mit `make all` und `make install`) 

```bash
export PATH="$PWD/bin:$PATH"
```

oder (Debug Build mit Xcode)

```bash
export PATH="$PWD/build/Debug/bin:$PATH"
```

oder (Release Build mit Xcode)
...
* **Seite für Downloads von `.mat` Dateien:** https://germansl.infinitenature.org/downloads/
